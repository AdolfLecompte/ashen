...
Window {
    ...
    property int shot: 0
    readonly property var names: ["session-open", "user-open"]

    Timer {
        interval: 700; running: true; repeat: true
        onTriggered: {
            var which = win.shot === 0 ? "sessionPicker" : "userPicker"
            var other = win.shot === 0 ? "userPicker" : "sessionPicker"
            var o = win.findByName(theme.item, other); if (o) o.popup.close()
            var c = win.findByName(theme.item, which); if (c) c.popup.open()
            win.contentItem.grabToImage(function (r) {
                r.saveToFile("pick-" + win.names[win.shot] + ".png")
                win.shot++
                if (win.shot >= 2) Qt.exit(theme.status === Loader.Ready ? 0 : 1)
            })
        }
    }
}
