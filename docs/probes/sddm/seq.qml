...
Window {
    ...
    property int shot: 0
    readonly property var names: ["seq1", "seq2", "seq3", "seq4", "seq6"]
    readonly property var words: ["a", "ab", "abc", "abcd", "abcdef"]

    Timer {
        interval: 700; running: true; repeat: true
        onTriggered: {
            var f = win.findByName(theme.item, "password")
            if (f) f.text = win.words[win.shot]
            win.contentItem.grabToImage(function (r) {
                r.saveToFile("shot-" + win.names[win.shot] + ".png")
                win.shot++
                if (win.shot >= 5) Qt.exit(theme.status === Loader.Ready ? 0 : 1)
            })
        }
    }
}
